...
- 门控规则版本为 `evidence-gate-v1`，参数由 `EvidenceSufficiencyConfig` 集中管理并在创建时校验：`min_vector_similarity=0.82`、`max_candidate_rank=3`、`max_original_rank=5`、`minimum_anchor_count=2`、`minimum_anchor_coverage=0.60`、`minimum_semantic_anchor_count=2`、`minimum_semantic_anchor_coverage=0.50`、`minimum_phrase_characters=5`、`minimum_distinct_sources=1`、`min_fts_similarity=0.50`、`min_semantic_similarity=0.60`、`numeric_context_characters=48`。规则版本当前固定为 `evidence-gate-v1`，未知版本及越界/非有限参数被拒绝。
...
- 默认支持候选仍要求 FTS 与 vector 原始 rank 均不晚于 5、融合后名次不晚于 3；正文需覆盖至少两个查询锚点且覆盖率不低于 0.60，或包含至少五个规范化字符的完整查询短语。第二十八批增加两条受限校准路径：FTS 与 vector 双路正文一致、rank/身份/数值/编号语境通过且相似度不低于 `0.50` 时记录 `FTS_EVIDENCE_VERIFIED_BELOW_VECTOR_THRESHOLD`；FTS 缺失时仅允许相似度不低于 `0.60`、至少两个语义锚点且覆盖率不低于 `0.50` 的 vector-only 候选，记录 `SEMANTIC_EVIDENCE_VERIFIED_BELOW_VECTOR_THRESHOLD`。查询中的编号必须在正文出现；显式数值必须以相同单位在正文锚点附近出现；文件名和 heading 不计正文覆盖；RRF/精确奖励/多样性分不作为证据门槛，不能单独绕过门控。
- 来源数按符合门槛候选的 distinct `file_id` 计算，默认至少 1；重复 Chunk 不会制造额外来源，单文件有效证据可以通过。复合问题按 `以及/并且/同时` 拆分，只有每个子句均能在候选正文中定位才支持；开放列举、缺失子句和无法确认的语义仍整体不足。两个以上支持来源出现不同数值或明确相反肯定/否定标记时整体拒绝。规则无法确认时按不足处理。
