...
- 状态：代码、定向测试和隔离浏览器门禁 `PASS`。Demo 可用性：Mock 主链路仍可启动；真实 DeepSeek 为 `PENDING`。阶段 5、阶段 6 的完整 V1 仍为 `PARTIAL`。进场分支 `feat/v1-bootstrap`，本地与远端 SHA 均为 `5dad7bff3f9561e92b552c2416052386ee015930`，工作区干净。
- 交付顺序已写入 `AGENTS.md` 与本进度：Demo 首先完成，完整 V1 以后精进。已有需求没有删除，也没有改标为 `PASS`。
- 生成模式：设置页可显式选择 Mock 或 DeepSeek 在线生成。默认 Mock。切换、打开聊天页和刷新都不发真实请求。在线模式展示“DeepSeek 在线生成、会外发当前问题与必要的少量证据”，并展示 2026-09-26 官方 `deepseek-flash` 高峰、缓存未命中费率下的保守估算。估算不是严格美元限额。未同意、未配置系统凭据或未勾选估算确认时，聊天发送被禁用；后端在调用前拒绝，Provider 次数为 0。Key 不进入前端响应。
- 外发边界：知识库在线请求输出上限 256 Token；输入按 1 字 1 Token 估算，并有 2048 字符上限。证据最多 2 段、摘录截短后才允许外发。资料不足仍由本地拒答，调用增量为 0。在线回答若没有命中已外发证据编号，记为 `CITATION_CONSTRAINT_FAILED`，不标成功，也不自动重试。冻结模型别名仍是 `deepseek-flash`，官方页当前对应 DeepSeek-V4.1-Flash，Base URL 仍是 `https://api.deepseek.com`。
- 脚本退出：`scripts/dev.ps1` 只清理本次启动的后端/前端。Ctrl+C 和 Stop-Process 的退出码 -1 视为正常停止并返回 0。其他退出码仍失败并清理本批进程。现场结束 8001 上的本批后端后，脚本退出码为 0，随后 8001/5174 释放。定向自测证明无关进程不被结束。无法从进程树区分“以 -1 自行退出”和“被 Stop-Process 结束”，因此只把 -1 当作操作者停止。
- 真实调用：本批没有读取或索要 Key，没有无人值守请求。真实 DeepSeek 保持 `PENDING`。用户以后在设置页自行保存 Key、确认外发版本、切到在线模式，并在点击前确认估算后，最多做一次连接探测和一次公开合成资料问题；不要自动重试，不要升级模型。
- 门禁：定向后端含 provider、聊天门禁、知识库正负例和运行时共 39 项通过；Ruff 与 Pyright 对本批文件为 0 错误。前端设置/聊天 Vitest 4 项通过，typecheck、lint、build 通过。OpenAPI 3.1 为 `64 schemas / 73 operations`。隔离浏览器确认 Mock 文案、在线外发文案、费用估算和未配置时输入禁用。没有重跑第 37 批全量，也没有把第 37 批成绩算进本批。
- 下一批唯一目标：做最小学习陪练的可演示主链路，继续默认 Mock，不把本批未发生的真实 DeepSeek 调用写成已连通。

### 第三十九批：最小学习陪练服务端闭环

- 状态：后端闭环代码与定向测试通过。前端学习页未接入，所以 Demo 主链路仍缺最小学习页和 Windows 端到端。阶段 5、6、7 的完整 V1 仍为 `PARTIAL`。真实 DeepSeek 保持 `PENDING`。进场分支 `feat/v1-bootstrap`，本地与远端 SHA 均为 `4562eac30cc85d05eb4c9995f534c6cc0176d5c0`，工作区干净。
- 交付：已有 `READY` 知识库可创建持久学习会话、一道单选题和一次作答反馈。公开样本唯一事实是“30 秒”。提交前不返回答案或摘录；提交后保存原答案、不同反馈和来源快照。幂等键不重复计分，提交后锁定。来源失效为 `SOURCE_INVALID`，不降级到普通聊天。资料不足或索引不可用时 Provider 调用数为 0。成功路径使用本地 Mock 规则，不调用聊天 Provider。
...
- 状态：`PARTIAL`
- 第四十三批完成对话历史的只读找回，第四十四批完成学习历史的只读找回，第四十五批完成首页最近学习卡片，第四十六批完成首页四个快捷操作，第四十七批完成首页最近知识库、最近文件和最近对话，第四十八批完成首页未回收资料计数和后台任务只读摘要。历史筛选和删除、设置、备份与完整恢复仍未验收。整页首页和阶段 8 仍为 `PARTIAL`。

### 第四十八批：首页真实概览与后台任务只读面板

- 状态：概览计数 `PASS`；任务摘要在浏览器核对了空态和已完成态，排队、运行、失败的浏览器画面为 `PENDING`，定向测试已覆盖；只读面板在空态和已完成态为 `PASS`。阶段 8 整体 `PARTIAL`。求职 Demo 维持第四十二批 `PASS`。真实 DeepSeek 仍为 `PENDING`。阶段 5、6、7 仍为 `PARTIAL`。进场 SHA `24287e47d3149af69045ebaf6a104f5229944ea3`。
- 口径：新增只读 `GET /api/v1/home/overview`。四个计数都排除 `deleted_at` 非空记录；文件计数包含解析失败，知识库计数包含空库和索引失败，对话计数包含普通对话和知识库对话，学习会话计数包含未完成、已完成和未能出题。任务排队是 `CREATED` 加 `QUEUED`，运行是 `RUNNING`，失败是 `FAILED`。进度只有在检查点能证明工作量时才显示百分比。
- 证据：后端定向 `2 passed`；首页四个 Vitest 文件串行 `14 passed`；相关 Ruff、Pyright、typecheck、lint、build 通过。隔离浏览器核对空库四计数为 0、导入后文件/知识库/对话为 1、回收文件后文件回到 0，点击文件数字进入文件页且任务数不变。报告见 `docs/test-reports/stage-48-home-overview.md`。
- 下一批唯一目标：把历史搜索、筛选和删除恢复收成同一次可验收交付，不把设置、备份或阶段 8 标成完成。

### 第四十七批：首页最近知识库、文件和对话

- 状态：最近知识库、最近文件、最近对话三栏各自 `PASS`；阶段 8 整体 `PARTIAL`。求职 Demo 维持第四十二批 `PASS`。真实 DeepSeek 仍为 `PENDING`。阶段 5、6、7 仍为 `PARTIAL`。进场 SHA `5c69d2ca906d870ea94cf25069bd892e1c60e1c4`。
- 排序：知识库和文件没有最近使用或最近打开时间，分别按 `updated_at` 显示“最近更新”。对话历史的 `updated_at` 来自 `last_active_at`，显示“最近活动”。知识库列表新增可选 `limit`，首页请求 4 条；文件和对话复用已有 `limit`。
- 证据：后端定向 `1 passed`；前端首页 Vitest `3 passed`，快捷操作和继续学习回归通过；typecheck、lint、build 通过。真实 Chrome 在隔离根核对空库、四条知识库上限、回收排除、失败文件确认前不重试，以及普通 Mock 对话。来源失效对话只在 Vitest 中验证。报告见 `docs/test-reports/stage-47-home-recent.md`。
- 下一批唯一目标：只读展示首页任务状态摘要，不把首页统计、设置或阶段 8 标成完成。

### 第四十六批：首页四个快捷操作

- 状态：本批四个快捷操作切片 `PASS`；阶段 8 整体 `PARTIAL`。求职 Demo 维持第四十二批 `PASS`。真实 DeepSeek 仍为 `PENDING`。阶段 5、6、7 仍为 `PARTIAL`。进场 SHA `dd504908156e7725c6ace2060f3710019b37759d`。
- 契约：没有新接口、迁移或 OpenAPI 变化。开始学习复用 `/learning/new`，AI 提问复用 `/chat`，导入文件复用 `/files`，创建知识库复用 `/knowledge-bases/new`。
- 证据：前端快捷操作 Vitest `3 passed`；typecheck、lint、build 通过。真实 Chrome 在两个隔离根分别核对空库引导和 READY 资料入口，点击返回后文件、知识库、对话、学习会话和 Attempt 数量不变。报告见 `docs/test-reports/stage-46-home-shortcuts.md`。
- 下一批唯一目标：在首页增加只读的最近知识库，不把统计、任务摘要、设置或阶段 8 标成完成。

### 第四十五批：首页继续学习入口

- 状态：本批首页继续学习切片 `PASS`；阶段 8 整体 `PARTIAL`。求职 Demo 维持第四十二批 `PASS`。真实 DeepSeek 仍为 `PENDING`。阶段 5、6、7 仍为 `PARTIAL`。进场 SHA `c5a39be40e1e6095826c580a0c6a423be4f92cf2`。
- 契约：复用 `GET /api/v1/history/learning-sessions`，首页只请求 `limit=1`。没有新接口、迁移或 OpenAPI 变化。
- 证据：前端首页 Vitest `5 passed`；typecheck、lint、build 通过。真实 Chrome 在隔离根完成空引导、未作答原会话、已作答反馈、刷新、后端重启和回收站拦截。报告见 `docs/test-reports/stage-45-home-learning.md`。
- 下一批唯一目标：补首页四个快捷操作，不把首页其余板块或阶段 8 标成完成。
