...
- 固定资料：`docs/test-data/stage5-fixed-ready/` 提供主库超时策略、相似干扰库策略和索引 READY 后移入回收站的范围验证资料；另有无成员诊断库验证未就绪错误。运行命令：`uv run python scripts/prepare_stage5_fixed_ready.py --data-dir "$env:TEM