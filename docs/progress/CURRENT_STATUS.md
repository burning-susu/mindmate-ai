...
- 当前批次状态：第四十八批在首页只读展示资料计数和后台任务状态。文件、知识库、对话、学习会话都按未回收记录计数；排队、运行、失败来自已有任务，失败原因只保留安全错误码或“失败详情已省略”。无任务时收起摘要。点击数字只打开已有列表。概览计数、任务摘要和只读面板按本批证据分别记录，整页首页和阶段 8 仍为 `PARTIAL`。第四十七批最近三栏、第四十六批快捷操作和第四十五批继续学习仍然有效。求职 Demo 维持第四十二批 `PASS`。真实 DeepSeek 仍为 `PENDING`。阶段 5、6、7 的完整 V1 仍为 `PARTIAL`。详细证据见 `docs/test-reports/stage-48-home-overview.md`。
...
- 本批状态：首页继续学习切片 `PASS`。整个阶段 8 仍为 `PARTIAL`。求职 Demo 维持第四十二批 `PASS`。真实 DeepSeek 仍为 `PENDING`。阶段 5、6、7 的完整 V1 仍为 `PARTIAL`。进场分支 `feat/v1-bootstrap`，本地与远端 SHA 均为 `c5a39be40e1e6095826c580a0c6a423be4f92cf2`，工作区干净。
- 入口：首页醒目区域读取已有 `GET /api/v1/history/learning-sessions?limit=1`。展示主题、资料范围、真实状态、已作答数量和最近学习时间。未作答且来源有效显示“继续学习”，已作答显示“查看反馈”，不显示“查看总结”。`SOURCE_INVALID` 写明来源原因，未作答只给“查看原会话”和历史页，已作答仍可打开原反馈。读取失败时隐藏旧卡片，提供重新加载和学习历史。没有学习记录时，“开始第一次学习”去知识库列表，“先导入资料”去文件页，并说明必须先选索引就绪且有可用文件的知识库。打开首页不创建会话、题目或 Attempt。没有改后端契约。
