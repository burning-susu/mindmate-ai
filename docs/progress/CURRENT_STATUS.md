...
- 当前批次状态：第四十六批把首页四个快捷操作接到已有流程。开始学习进入学习准备页并只列出索引就绪的知识库；AI 提问进入新对话并选择普通对话或已就绪知识库；导入文件进入带上传控件的文件页；创建知识库打开原有新建表单。未提交或未发送时不新建文件、知识库、对话、学习会话或 Attempt。本批四个快捷操作切片 `PASS`，整个阶段 8 仍为 `PARTIAL`。第四十五批继续学习切片仍然有效。求职 Demo 维持第四十二批 `PASS`，本批没有重跑空库上传全链路。真实 DeepSeek 仍为 `PENDING`。阶段 5、6、7 的完整 V1 仍为 `PARTIAL`。详细证据见 `docs/test-reports/stage-46-home-shortcuts.md`。
