...
- 当前批次状态：第四十七批在首页只读展示最近知识库、最近文件和最近对话。知识库按更新时间取最多 4 个，文件按更新时间取最多 5 个，对话按最近活动时间取最多 5 个；回收对象不出现。只有索引 `READY` 且有可用文件时才给出学习和提问入口。三栏各自 `PASS`，整个阶段 8 仍为 `PARTIAL`。第四十六批快捷操作和第四十五批继续学习仍然有效。求职 Demo 维持第四十二批 `PASS`。真实 DeepSeek 仍为 `PENDING`。阶段 5、6、7 的完整 V1 仍为 `PARTIAL`。详细证据见 `docs/test-reports/stage-47-home-recent.md`。
