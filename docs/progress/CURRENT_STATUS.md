...
- 当前批次状态：第四十五批在首页放了只读的最近学习卡片。空记录会引导去知识库和文件导入；未作答且来源有效时“继续学习”只打开原会话；已作答显示“查看反馈”；来源进入回收站后首页不再提供继续，并写明原因。本批首页继续学习切片 `PASS`，整个阶段 8 仍为 `PARTIAL`。第四十四批学习历史切片仍然有效。求职 Demo 维持第四十二批 `PASS`，本批没有重跑空库上传全链路。真实 DeepSeek 仍为 `PENDING`。阶段 5、6、7 的完整 V1 仍为 `PARTIAL`。详细证据见 `docs/test-reports/stage-45-home-learning.md`。
