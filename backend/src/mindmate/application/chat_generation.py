...
def recover_interrupted_chat_operations(session: Session) -> int:
    ...
    for operation in operations:
        ...
        if task is not None and task.status in {"RUNNING", "QUEUED"}:
            ...
            add_event(session, task, "INTERRUPTED", {"reason": "APPLICATION_RESTART"})
        recovered += 1
    session.commit()
    return recovered
...
class ChatGenerationWorker:
    ...
    def _process_task(self, task_id: str) -> None:
        with self._session_factory() as session:
            ...
            if assistant is None or answer is None:
                self._fail_operation(session, task, operation, None, "CHAT_STATE_INVALID", "聊天生成状态不完整。")
                session.commit()
                return
            ...
            task.checkpoint_json = copy.deepcopy(task.checkpoint_json or {})
            task.checkpoint_json["phase"] = "GENERATING"
            ...
            try:
                if getattr(provider, "requires_external_transfer", True):
                    ...
                    try:
                        api_key = self._credential_store_getter().get_secret(
                ...
                self._fail_after_provider_error(task_id, exc)
                return
            except Exception:
                ...
                api_key = None
        self._complete_operation(task_id, response)

    def _complete_operation(self, task_id: str, response: ChatResponse) -> None:
        with self._session_factory() as session:
            task = session.get(BackgroundTask, task_id)
            operation = session.scalar(select(AiOperation).where(AiOperation.task_id == task_id))
            ...
            if not content or len(content) > MAX_OUTPUT_CHARS:
            ...
            add_event(session, task, "COMPLETED", {"operation_id": operation.operation_id})
            session.commit()

    def _fail_after_provider_error(self, task_id: str, error: ProviderRequestError) -> None:
        with self._session_factory() as session:
            task = session.get(BackgroundTask, task_id)
            operation = session.scalar(select(AiOperation).where(AiOperation.task_id == task_id))
            if task is None or operation is None or task.status != "RUNNING":
                return
            self._fail_operation(session, task, operation, None, error.code, error.detail)
            session.commit()
    ...
    def _fail_operation(
        ...
        now = utc_now()
        assistant = assistant or session.get(Message, operation.assistant_message_id)
        answer = session.scalar(
            select(AnswerVersion)
            .where(AnswerVersion.assistant_message_id == operation.assistant_message_id)
            .order_by(AnswerVersion.version_number.desc())
        ...
        add_event(session, task, "FAILED", {"error_code": code[:100]})
    ...
    def _mark_interrupted(
        ...
        task.row_version += 1
        finish_attempt(session, task.task_id, "INTERRUPTED", task.error_summary)
        add_event(session, task, "INTERRUPTED", {"reason": "APPLICATION_STOPPING"})
